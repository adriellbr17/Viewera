...
export function createServer({ origins = ['http://127.0.0.1:8787', 'http://localhost:8787', 'http://tauri.localhost', 'https://tauri.localhost', 'tauri://localhost'], accessKey = '', iceProvider = async () => [], relayOnly = false } = {}) {
  ...
  wss.on('connection', ws => {
    ...
    Object.assign(ws, { id: randomUUID(), room: null, alive: true, count: 0, window: Date.now() });
    ...
    async function handle(raw, binary) {
      ...
      try {
        ...
        if (m.type === 'create' || m.type === 'join') {
          ...
          if (typeof m.name !== 'string' || !m.name.trim() || m.name.length > 32) throw Error('Nome inválido (1–32 caracteres)');
          ...
          if (accessKey && (typeof m.accessKey !== 'string' || m.accessKey.length > 256 || !timingSafeEqual(digest(m.accessKey), digest(accessKey)))) {
          ...
          ws.room = code; ws.name = m.name.trim(); room.members.set(ws.id, ws);
